...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const handleDeleteEpic = (e: React.MouseEvent, epicId: string) => {
    e.stopPropagation();
    if (confirm('Delete this Epic and all its contents? This cannot be undone.')) {
      onDeleteEpic(epicId);
    }
  };
  ...
  return (
    <div className="w-full md:w-80 flex-shrink-0 flex flex-col bg-white dark:bg-[#020617] md:border-r border-slate-200 dark:border-slate-900 h-full overflow-hidden transition-all duration-300">
      {/* Archive Header */}
      <div className="p-5 md:p-6 flex items-center justify-between border-b border-slate-50 dark:border-slate-900/50 mb-4 bg-slate-50/30 dark:bg-slate-900/10">
        <div className="flex items-center gap-3 md:gap-4">
          <Link to="/" title="Back to Dashboard" className="p-2 -ml-2 text-slate-400 hover:text-primary transition-all rounded-xl hover:bg-white dark:hover:bg-slate-800 shadow-sm border border-transparent hover:border-slate-100 dark:hover:border-slate-700">
        ...
        <button 
          ...
          className="p-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 hover:text-primary transition-all active:scale-95 shadow-sm"
        >
      ...
      {/* Index Tree */}
      <div className="flex-1 overflow-y-auto px-3 md:px-4 pb-10 space-y-2 no-scrollbar">
        {filteredEpics.map(epic => (
          ...
          >
            <div 
              className={`flex items-center group px-3 py-2 rounded-xl cursor-pointer transition-all ${epic.isOpen ? 'bg-slate-50 dark:bg-slate-900/80 text-slate-900 dark:text-slate-200 shadow-sm border border-slate-100 dark:border-slate-800' : 'text-slate-400 dark:text-slate-600 border border-transparent hover:bg-slate-50/50 dark:hover:bg-slate-900/40'}`}
            ...
            >
              ...
              {/* Epic Controls - Visible on Hover */}
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  onClick={(e) => handleStartRename(e, epic)}
                  title="Rename Epic"
                  className="p-1.5 text-slate-400 hover:text-primary transition-colors rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700"
                >
                ...
                <button 
                  ...
                  title="Delete Epic"
                  className="p-1.5 text-slate-400 hover:text-rose-500 transition-colors rounded-lg bg-white dark:bg-slate-800 shadow-sm border border-slate-100 dark:border-slate-700"
                >
            ...
            {epic.isOpen && (
              <div className="ml-5 space-y-1 mt-1 border-l-2 border-slate-100 dark:border-slate-800/50 pl-2 animate-in slide-in-from-left-2 duration-200">
                {epic.stories.map(story => (
                  <div 
                    key={story.id}
                    draggable
                    onDragStart={(e) => handleDragStart(e, story.id)}
                    className={`flex items-center group px-3.5 py-2 rounded-xl cursor-pointer transition-all relative border-2
                  ...
                  >
                    {activeStoryId === story.id && (
                      <div className="absolute -left-[10.5px] top-1/2 -translate-y-1/2 w-[3px] h-4 bg-primary rounded-full"></div>
                    )}
                    <Icons.FileText className={`mr-2.5 w-3.5 h-3.5 ${activeStoryId === story.id ? 'text-primary' : 'text-slate-200 dark:text-slate-800'}`} />
                    <span className="text-[11px] md:text-[12px] truncate font-medium flex-1 tracking-tight">{story.title}</span>
                    
                    <button 
                      onClick={(e) => handleDeleteStory(e, story.id)}
                      title="Delete Story"
                      className="opacity-0 group-hover:opacity-100 p-1.5 hover:text-rose-500 transition-all rounded-lg bg-white/50 dark:bg-slate-800/50"
                    >
                      <Icons.Trash className="w-3 h-3" />
                    </button>
                  </div>
                ...
                <button 
                  ...
                  className="w-full flex items-center gap-3 px-3.5 py-2.5 text-[9px] font-black text-slate-300 dark:text-slate-800 hover:text-primary transition-all uppercase tracking-widest border border-dashed border-slate-200 dark:border-slate-800 hover:border-primary/20 rounded-xl mt-1"
                >
        ...
        {filteredEpics.length === 0 && (
          <div className="py-20 text-center opacity-30 flex flex-col items-center gap-3">
            <Icons.Folder className="w-10 h-10 text-slate-300" />
      ...
      {/* Footer */}
      <div className="p-6 border-t border-slate-100 dark:border-slate-900/60 items-center justify-between flex shrink-0 bg-slate-50/50 dark:bg-slate-900/20">
         <div className="flex items-center gap-3">
           <Icons.RotateCcw className="w-4 h-4 text-slate-300 dark:text-slate-800" />
           <span className="text-[9px] font-black text-slate-400 dark:text-slate-800 uppercase tracking-[0.5em] hover:text-slate-600 cursor-pointer transition-colors">Session</span>
         </div>
         <button 
            onClick={onToggleDarkMode}
            title={isDarkMode ? "Switch to Light Mode" : "Switch to Dark Mode"}
            className="p-2.5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-400 dark:text-slate-500 hover:text-primary transition-all active:scale-90 shadow-sm"
         >
           {isDarkMode ? (
